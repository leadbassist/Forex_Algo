...
class MAResult:
    ...
    def result_ob(self):
        return dict(
            ...
            mean=int(self.df_trades.GAIN.mean()),
            min=int(self.df_trades.GAIN.min()),
            max=int(self.df_trades.GAIN.max()),
            ma_l=self.ma_l,
            ma_s=self.ma_s,
        )
...
def assess_pair(price_data, ma_l, ma_s, instrument):
    ...
    return MAResult(df_trades, instrument.name, ma_l, ma_s)
...
def analyse_pair(instrument, granularity, ma_long, ma_short):
    ...
    for ma_l in ma_long:
        for ma_s in ma_short:
            ...
            result = assess_pair(
                price_data, get_ma_col(ma_l), get_ma_col(ma_s), instrument
            )
