...
def add_chart(pair, cross, df, writer):
    ...
    chart = get_line_chart(
        workbook, 1, df.shape[0], 11, 12, f"GAIN_C for {pair} { cross}", pair
    )
...
def add_pair_charts(df_ma_res, df_ma_trades, writer):
    ...
    for _, row in df_temp.iterrows():
        dft = df_ma_trades[
            (df_ma_trades.cross == row.cross) & (df_ma_trades.cross == row.cross)
        ]
        dft[cols].to_excel(
            writer, sheet_name=row.pair, index=False, startrow=0, startcol=11
        )
...
def prepare_data(df_ma_res, df_ma_trades):
    df_ma_res.sort_values(
        by=["pair", "total_gain"], ascending=[True, False], inplace=True
    )
...
# this will run from main.py, which is in ROOT folder
# so, file location is from ROOT folder perspective
...
# the below is being run from "here"..as in...from "simulation" folder
# so, file location is defined from simulation folder perspective
