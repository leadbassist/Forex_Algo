...
def get_trade_units(api: OandaApi, pair, signal, loss, trade_risk, log_message):
    ...
    if price == None:
        log_message("get_trade_units() price is None? investigate", pair)
        return False
    ...
    per_pips_loss = trade_risk / num_pips
    units = per_pips_loss / (conv * pipLocation)

    log_message(f"{pipLocation} {num_pips} {per_pips_loss} {units}", pair)
