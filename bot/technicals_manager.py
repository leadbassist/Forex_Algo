...
def process_candles(df: pd.DataFrame, pair, trade_settings: TradeSettings, log_message):
    ...
    # to reset the index in the df
    df.reset_index(drop=True, inplace=True)
    # add some columns in df
    df["PAIR"] = pair
...
def fetch_candles(
    pair, row_count, candle_time, granularity, api: OandaApi, log_message
):
    ...
    if df.iloc[-1].time != candle_time:
        log_message(
            f"tech_manager fetch_candles {df.iloc[-1].time} failed to get candles", pair
        )
        return None
...
def get_trade_decision(
    ...
    log_message(
        f"tech_manager: max_rows:{max_rows} candle_time:{candle_time} granularity{granularity}",
        pair,
    )
