...
def save_file(final_df: pd.DataFrame, file_prefix, granularity, pair):
    ...
    print(
        f"*** {pair} {granularity} {final_df.time.min()} {final_df.time.max()} ==> {final_df.shape[0]} candles saved***"
    )


def fetch_candles(
    pair, granularity, date_f: dt.datetime, date_t: dt.datetime, api: OandaApi
):
    attempts = 0
...
def collect_data(pair, granularity, date_f, date_t, file_prefix, api: OandaApi):
    ...
    while to_date < end_date:
        ...
        if candles is not None:
            ...
            print(
                f"{pair} {granularity} {from_date} {to_date} ==> {candles.shape[0]} candles loaded"
            )
        else:
